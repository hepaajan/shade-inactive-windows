...
function enable() {
	...
	/* Don't use our shader on certain window types */
	...
	/* Verify if window has our effect shader */
	...
	/* Toggle shader on focus */
	function focus(the_window) {
		global.get_window_actors().forEach(function(wa) {
			
			/* Verify our effect shader */
			verifyShader(wa);
			if (!wa._inactive_shader)
				return;

			/* Fade in/out our shader */
	...
	/* Catch new created windows and attach events */
	...
	/* Loop existing windows, attach events and shaders */
	global.get_window_actors().forEach(function(wa) {
		var meta_win = wa.get_meta_window();
		if (!meta_win)
			return;
...
function disable() {
	...
	/* Loop existing windows, disconnect events and destroy shaders */
	global.get_window_actors().forEach(function(wa) {
